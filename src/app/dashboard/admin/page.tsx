...
import { Users, Trash2, CheckCircle, AlertTriangle, Loader2, Check, X, Filter, LogOut } from "lucide-react"
...
export default function AdminDashboard() {
    ...
    // Fetch AI summary when stats change
    useEffect(() => {
        if (stats) {
    ...
    const handleVerifyTask = async (taskId: string, approved: boolean) => {
        try {
            ...
            mutate({ stats: stats as DashboardStats, tasks: updatedTasks }, false);
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 pb-20">
            ...
            <div className="md:hidden flex items-center justify-between pb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <h1 className="text-xl font-bold tracking-tight text-foreground leading-tight">{user?.name?.split(' ')[0] || "Admin"}</h1>
                    </div>
                </div>
            ...
            <div className="sticky top-0 md:top-0 z-40 bg-background/80 md:bg-transparent backdrop-blur-xl md:backdrop-blur-none pb-2 pt-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:pb-0 sm:pt-0 border-b border-border/40 md:border-none md:flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center justify-between w-full md:w-auto gap-2">
                    <div className="flex items-center gap-2">
                        <Select value={dateRange} onValueChange={setDateRange}>
                            <SelectTrigger className="w-[140px] sm:w-[180px] h-9 sm:h-10 rounded-full sm:rounded-md bg-muted/50 border-transparent focus:border-primary/30 text-[13px] sm:text-sm font-semibold tracking-tight">
                    ...
                    <Button size="sm" onClick={handleRefresh} disabled={isRefreshing} className="ml-auto rounded-full sm:rounded-md shadow-md h-9 sm:h-10 px-4">
                        {isRefreshing ? <Loader2 className="h-4 w-4 animate-spin md:mr-2" /> : <RefreshCcw className="h-4 w-4 md:mr-2" />}
                        <span className="hidden md:inline font-semibold">{isRefreshing ? "Refreshing..." : "Refresh"}</span>
                    </Button>
                </div>
            </div>
            ...
            {/* KPI Cards (Scrollable on mobile) */}
            <div className="flex overflow-x-auto pb-4 pt-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:grid sm:grid-cols-2 lg:grid-cols-4 sm:gap-6 gap-3 snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                ...
                <div className="min-w-[85vw] sm:min-w-0 snap-center">
                    <KpiCard
                        ...
                        className="border-orange-200 dark:border-orange-900/50 rounded-2xl"
                    />
                </div>
                <div className="min-w-[85vw] sm:min-w-0 snap-center">
                    <KpiCard
            ...
            <motion.div
                ...
                className="mb-8"
            >
                <div className="relative overflow-hidden rounded-[1.5rem] border border-blue-200 dark:border-blue-900/50 bg-gradient-to-br from-blue-50 to-indigo-50/50 dark:from-blue-950/20 dark:to-indigo-950/10 p-6 sm:p-8 shadow-sm">
                    ...
                    <div className="relative z-10 flex flex-col sm:flex-row gap-5 items-start">
                        ...
                        <div className="flex-1 space-y-2">
                            <h3 className="text-lg font-bold text-blue-900 dark:text-blue-100 flex items-center gap-2 tracking-tight">
                                AI City Insights
                                <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-sm bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300">Live</span>
                            </h3>
                            <div className="min-h-[48px] flex items-center">
                                {isGeneratingAi ? (
                                    ...
                                    <p className="text-[15px] font-medium leading-relaxed text-blue-800/90 dark:text-blue-200/80 max-w-4xl">
                                        {aiSummary || "Insufficient data to generate insights at this time."}
                                    </p>
                                )}
                            </div>
                        </div>
            ...
            <div className="grid gap-6 grid-cols-1 lg:grid-cols-12">
                ...
                <Card className="lg:col-span-6 xl:col-span-6 glass-card">
                    ...
                    <CardContent className="w-full p-4">
                        <div className="h-auto min-h-[350px] md:h-[350px] w-full">
